...
def run_instr(reg_value, instr, amount):
  if instr == 'inc':
    return reg_value + amount
  if instr == 'dec':
    return reg_value - amount
...
def solve(input):
  ...
  highest_value_evar_lol = 0

  # Run instructions and build history
  history = {}
  for line in lines:
    ...
    reg = matches.group(1)
    instr = matches.group(2)
    amount = int(matches.group(3))
    test_reg = matches.group(4)
    operator = matches.group(5)
    test_val = int(matches.group(6))

    if reg not in history:
      history[reg] = 0
    if test_reg not in history:
      history[test_reg] = 0

    if (operator == '>' and history[test_reg] > test_val) or \
       (operator == '<' and history[test_reg] < test_val) or \
       (operator == '<=' and history[test_reg] <= test_val) or \
       (operator == '>=' and history[test_reg] >= test_val) or \
       (operator == '==' and history[test_reg] == test_val) or \
       (operator == '!=' and history[test_reg] != test_val):
      history[reg] = run_instr(history[reg], instr, amount)

    if history[reg] > highest_value_evar_lol:
      highest_value_evar_lol = history[reg]

  return highest_value_evar_lol
