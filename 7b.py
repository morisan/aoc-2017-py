...
input = """bqyqwn (68) -> wscqe, cwxspl, syogw, xnxudsh
ddswb (34)
hnkvw (320)
...
def get_total_node_weight(lines, search_instr):
  for line in lines:
    parts = line.split(' -> ')

    p = re.compile("(\w+) \((\d+)\)")
    matches = p.search(parts[0])
    instr = matches.group(1)
    node_weight = int(matches.group(2))

    if instr == search_instr:
      if len(parts) == 2:
        above_instrs = parts[1].split(', ')

        node_total_weight = node_weight

        test_weight = None
        for ai in above_instrs:
          child_weight = get_total_node_weight(lines, ai)
          node_total_weight += child_weight

          if not test_weight:
            test_weight = child_weight

          if test_weight != child_weight:
            weight_difference = child_weight - test_weight

            offending_instr = ai
            for line in lines:
              chunks = line.split()
              if chunks[0] == offending_instr:

                # Dirty early exit :-/
                print int(chunks[1][1:-1]) - weight_difference
                exit()

        return node_total_weight
      else:
        return node_weight
...
def solve(input):
  ...
  for line in lines:
    ...
    if len(parts) == 2:
      possible_base_instrs.append(instr)
      all_linked_instrs.extend(parts[1].split(', '))

  intersection = list(set(possible_base_instrs) & set(all_linked_instrs))

  head = list(set(possible_base_instrs) - set(intersection))[0]

  get_total_node_weight(lines, head)
  return
